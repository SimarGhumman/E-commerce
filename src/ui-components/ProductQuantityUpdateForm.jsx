...
import { updateProductQuantity } from "../graphql/mutations";
...
export default function ProductQuantityUpdateForm(props) {
  const {
    id: idProp,
    productQuantity: productQuantityModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = productQuantityRecord
      ? {
          ...initialValues,
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getProductQuantity.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
      ...
      const orderProductsIdRecord = record ? record.orderProductsId : undefined;
      const orderRecord = orderProductsIdRecord
        ? (
            await client.graphql({
              query: getOrder.replaceAll("__typename", ""),
              variables: { id: orderProductsIdRecord },
            })
          )?.data?.getOrder
        : undefined;
      setOrderProductsId(orderProductsIdRecord);
      setSelectedOrderProductsIdRecords([orderRecord]);
      const shoppingCartProductsIdRecord = record
        ? record.shoppingCartProductsId
        : undefined;
      const shoppingCartRecord = shoppingCartProductsIdRecord
        ? (
            await client.graphql({
              query: getShoppingCart.replaceAll("__typename", ""),
              variables: { id: shoppingCartProductsIdRecord },
            })
          )?.data?.getShoppingCart
        : undefined;
      setShoppingCartProductsId(shoppingCartProductsIdRecord);
      setSelectedShoppingCartProductsIdRecords([shoppingCartRecord]);
      const productQuantitiesIdRecord = record
        ? record.productQuantitiesId
        : undefined;
      const productRecord = productQuantitiesIdRecord
        ? (
            await client.graphql({
              query: getProduct.replaceAll("__typename", ""),
              variables: { id: productQuantitiesIdRecord },
            })
          )?.data?.getProduct
        : undefined;
      setProductQuantitiesId(productQuantitiesIdRecord);
      setSelectedProductQuantitiesIdRecords([productRecord]);
      setProductQuantityRecord(record);
    };
    queryData();
  }, [idProp, productQuantityModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          quantity,
          orderProductsId: orderProductsId ?? null,
          shoppingCartProductsId: shoppingCartProductsId ?? null,
          productQuantitiesId: productQuantitiesId ?? null,
        };
        ...
        try {
          ...
          const modelFieldsToSave = {
            ...
            orderProductsId: modelFields.orderProductsId ?? null,
            shoppingCartProductsId: modelFields.shoppingCartProductsId ?? null,
            productQuantitiesId: modelFields.productQuantitiesId ?? null,
          };
          await client.graphql({
            query: updateProductQuantity.replaceAll("__typename", ""),
            variables: {
              input: {
                id: productQuantityRecord.id,
                ...modelFieldsToSave,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ProductQuantityUpdateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
      ...
      >
        <Autocomplete
          ...
          defaultValue={shoppingCartProductsId}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Autocomplete
          ...
          defaultValue={productQuantitiesId}
          onChange={(e) => {
            let { value } = e.target;
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || productQuantityModelProp)}
