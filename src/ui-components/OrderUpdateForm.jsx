...
export default function OrderUpdateForm(props) {
  ...
  const [linkedOrderItems, setLinkedOrderItems] = React.useState([]);
  const canUnlinkOrderItems = false;
  React.useEffect(() => {
    const queryData = async () => {
      ...
      const linkedOrderItems = record?.OrderItems?.items ?? [];
      setLinkedOrderItems(linkedOrderItems);
      setOrderRecord(record);
    };
    queryData();
  }, [idProp, orderModelProp]);
  React.useEffect(resetStateValues, [orderRecord, userID, linkedOrderItems]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const orderItemsToLink = [];
          const orderItemsToUnLink = [];
          const orderItemsSet = new Set();
          const linkedOrderItemsSet = new Set();
          OrderItems.forEach((r) =>
            orderItemsSet.add(getIDValue.OrderItems?.(r))
          );
          linkedOrderItems.forEach((r) =>
            linkedOrderItemsSet.add(getIDValue.OrderItems?.(r))
          );
          linkedOrderItems.forEach((r) => {
            if (!orderItemsSet.has(getIDValue.OrderItems?.(r))) {
              orderItemsToUnLink.push(r);
            }
          });
          OrderItems.forEach((r) => {
            if (!linkedOrderItemsSet.has(getIDValue.OrderItems?.(r))) {
              orderItemsToLink.push(r);
            }
          });
          orderItemsToUnLink.forEach((original) => {
            if (!canUnlinkOrderItems) {
              throw Error(
                `OrderItem ${original.id} cannot be unlinked from Order because orderID is a required field.`
              );
            }
            promises.push(
              client.graphql({
          ...
          orderItemsToLink.forEach((original) => {
            promises.push(
              client.graphql({
                query: updateOrderItem.replaceAll("__typename", ""),
                variables: {
